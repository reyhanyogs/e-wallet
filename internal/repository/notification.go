...
func (r *notificationRepository) FindByUser(ctx context.Context, user int64) (notifications []domain.Notification, err error) {
	dataset := r.db.From("notifications").Where(goqu.Ex{
		"user_id": user,
...
func (r *notificationRepository) Insert(ctx context.Context, notification *domain.Notification) error {
	executor := r.db.Insert("notifications").Rows(goqu.Record{
...
func (r *notificationRepository) Update(ctx context.Context, notification *domain.Notification) error {
	executor := r.db.Update("notifications").Where(goqu.Ex{
		"id": notification.ID,
	}).Set(goqu.Record{
		"title":   notification.Title,
		"body":    notification.Body,
